...
# Last Change: Thu Dec 20 14:11:05 2012
...
# Add the half light and convert to Kpc
                    ...
                    print icd[i][0], gini[j][6],gini[j][6]*astCalc.da(icd[i][7])*1000./206265
    ...
    sc1 = f1s1.scatter(plt_matrix[:,1], plt_matrix[:,2],
                    #c='#F28500', s=50)#, cmap='spectral')
    c=plt_matrix[:,0], s=50, cmap='spectral')
    ...
    for label, x, y in zip(plt_matrix[:,3], plt_matrix[:,1], plt_matrix[:,2]):
        for galaxy, x1, y1 in zip(galaxylist, x_coor, y_coor):
            if galaxy == label:
                ...
                image =\
                read_png('/Users/steven/Projects/galaxy_icd/images/pngs_contours_halflight/'
...
#                 fc='yellow', alpha=0.5),arrowprops=dict(arrowstyle='->'))
    ...
    f1s1.set_xlabel(r"$R_{1/2} [kpc]$",fontsize=18)
