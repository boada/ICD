...
def mk_image(galaxy):
    ...
    img = img_scale.asinh(i_img, scale_min=-0.1*i_lim, scale_max=i_lim,
            non_linear=0.5)
    return img
...
x = [galaxy.clumps for galaxy in galaxies]
y = [galaxy.ICD_IH *100 for galaxy in galaxies]
ll = 0
ul= 3
#bins_x =pyl.linspace(ul, ll, 10)
bins_x = pyl.arange(0,6)
bins_y = pyl.linspace(0, 50, 10)

grid = []

#for i in range(bins_x.size-1):
for i in range(bins_x.size):
    xmin = bins_x[i]
    #xmax = bins_x[i+1]
    for j in  range(bins_y.size-1):
        ...
        cond=[cond1 and cond3 and cond4 for cond1, cond3,
            cond4 in zip(x==xmin, y>=ymin, y<ymax)]

        grid.append(galaxies.compress(cond))

# Put the grid together
F = pyl.figure(1, figsize=(4, 6))
grid1 = axes_grid.ImageGrid(F, 111, nrows_ncols=(9,6), axes_pad=0.05,
